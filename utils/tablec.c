...
void
tablec_init(struct tablec_ht *tablec, size_t max_capacity)
{
    ...
    while (max_capacity--) {
        ...
        tablec->buckets[max_capacity].array =
            malloc(sizeof(struct tablec_buckets_array));
...
size_t
_tablec_hash(struct tablec_ht *tablec, char *key)
{
    size_t hash = 0, i = 0;
...
void
tablec_resize(struct tablec_ht *tablec, size_t new_max_capacity)
{
    ...
    if (tablec->length != 0)
        while (i--) {
            ...
            if (tablec->buckets[i].length == 1) {
                tablec_set(&newHashtable, tablec->buckets[i].array[0].key,
                           tablec->buckets[i].array[0].value);
            ...
            while (tablec->buckets[i].capacity-- - 1) {
                if (tablec->buckets[i]
                        .array[tablec->buckets[i].capacity - 1]
                        .key)
                    tablec_set(&newHashtable,
                               tablec->buckets[i]
                                   .array[tablec->buckets[i].capacity - 1]
                                   .key,
                               tablec->buckets[i]
...
tablec_set(struct tablec_ht *tablec, char *key, void *value)
{
    ...
    if (tablec->buckets[hash].length == 0) {
        if (tablec->buckets[hash].array[0].key == NULL) {
            ...
            newArray = malloc(sizeof(struct tablec_buckets_array) * 2);
        ...
        newArray = calloc(sizeof(struct tablec_buckets_array)
                              * (tablec->buckets[hash].capacity * 2),
                          1);
        while (i--) {
...
tablec_get(struct tablec_ht *tablec, char *key)
{
    ...
    while (i--) {
        if (tablec->buckets[hash].array[i].key != NULL
            && strcmp(tablec->buckets[hash].array[i].key, key) == 0)
...
long
tablec_full(struct tablec_ht *tablec)
{
    return tablec->capacity == tablec->length
               ? (long)-1
               : (long)tablec->capacity - (long)tablec->length;
}

void
tablec_cleanup(struct tablec_ht *tablec)
{
